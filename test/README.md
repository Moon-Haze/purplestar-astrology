...
>
> 现在比对器改用 `lunar-typescript` 独立换算，并由层 3 用 iztro 自身的 `horoscope()`
...
### 7. 预言机也有边界：`lunar-lite` 的「农历→公历」有 17 处自相矛盾

2026-09 把农历引擎由 `lunar-javascript` 换成同作者的 `lunar-typescript`（1.7.7 → 1.8.6）。
换的是**换算引擎**，而农历日直接决定紫微定位 → 十二宫，抽样 300 条基准不足以证明行为不变，
故把旧包临时装到 `/tmp` 与新包**逐日对拍**：

| 对拍方向 | 规模 | 分歧 |
| --- | --- | --- |
| 公历→农历（`getLunarInfo` 的链） | 73,414 个日（1900-01-01 ~ 2100-12-31） | **0** |
| 农历→公历（CLI `--lunar` 的链） | 同域逐日往返 | **0** |
| 闰月结构（天数与存在性） | 全部 74 个闰月 | **0** |

即这次换包**对本项目行为零影响**，不是「大致兼容」。

对拍时另拿 iztro 自带的 `lunar-lite` 作第三方对照，撞出它的一个缺陷：**它的 `lunar2solar`
与它自己的 `solar2lunar` 互相矛盾**。1900–2100 间有 **17 个闰月**如此 —— `lunar-lite` 把
1906-06-21 算成「闰四月三十」，却拒绝把「闰四月三十」换回公历（报
`only 29 days in lunar year 1906 month 4`）。17 处完整清单（全部是闰月三十）：
1906 闰 4、1914 闰 5、1919 闰 7、1925 闰 4、1936 闰 3、1938 闰 7、1941 闰 6、1944 闰 4、
1952 闰 5、1955 闰 3、2017 闰 6、2036 闰 6、2047 闰 5、2050 闰 3、2055 闰 6、2058 闰 4、2093 闰 6。

**对本项目零影响**，两条独立原因：

- 本项目两个方向都用 `lunar-typescript`，上述 17 天**往返全部成立**（实测失败 0 处）
- iztro 虽也调 `lunar-lite` 的 `lunar2solar`（`FunctionalAstrolabe` 的 `yearlyList` /
  `monthlyList`），但实参的「日」只可能是 `1` 或 `16`（`yearlyList` 固定用六月初一），
  **永远取不到三十**，故那 17 天不会被请求；而 `byLunar` / `getSignByLunarDate`
  这两个可能撞上的入口，本项目从不调用（只用 `astro.bySolar`）

> ⚠️ **可操作的教训**：别拿 `lunar-lite` 当 `--lunar`（农历→公历）方向的预言机 ——
> 那 17 天的失败是**它的**缺陷，不是本项目的回归，照用会打出假红灯。
> 这与第 5、6 节是同一主题的另一面：**选预言机要知道它的边界**，
> 不是「随便找个第三方就叫独立验证」。
...
以下七处**没有基准可依**，只能靠层 2 的手工基准与 CLI 的 `selftest` 兜底：
...
| **合盘** | 样本是单人盘 |
| **流年/流月四化** | 样本是静态盘，无「流年」「流月」概念。层 3 已用三条独立路径核对（见下） |

层 2 已为前四项手工构造了断言（含用 `lunar-typescript` 独立换算互证）。

后三项在 2026-09 补上了覆盖，但都**不是正确性基准** —— 期望值来自本项目自己的复算：
...
- **流年/流月四化**：三合派三层四化的两动态层（`getLiuNianSiHua` 公历取模、
  `getLiuYueSiHua` 五虎遁）。此前只有「`--liunian` 非数字报错」一条边角断言，
  两函数的**算术**零覆盖。期望值走**三条互不相干的路径**：

  | 被测 | 独立路径 |
  | --- | --- |
  | 五虎遁月干 | 口诀独立表（甲己之年丙作首……120 格逐格写死，与实现的 `startStemOfYin` 零共享） |
  | 流年年干 | `lunar-typescript` 年柱（1924–2100 逐年，取年中日期避开立春/春节两个切年界） |
  | 流年年干 | 万年历固定向量（1900 庚 / 1984 甲 / 2024 甲等纸面常识写死，防 lunar 与实现同错） |

  层 2 另钉三条**只有端到端才能钉住的口径**：流月由流年干推**不串生年干**
  （生年庚 × 流年甲 → 正月月干丙，串了会得戊）、生年（农历年干）与流年（公历取模）
  **刻意分家**（1990-01-15 出生的【生年四化】年干己与【1990 流年四化】年干庚同屏）、
  `--json` 的 `located` 落宫与本进程按星名直接找宫逐项一致。

  反向注入 4 种失效模式（五虎遁表改一格 / 流年取模偏移一位 / 流月串用生年干 /
  月序偏移一位），全部被抓住、0 漏网 —— 其中「流月串用生年干」正是合盘那组
  「参数串台」的同一形态，专挑生年干 ≠ 流年干的样本才钉得住。

> 这三处此前是**全仓零覆盖**或仅边角覆盖，也正是宫名口径切换时会静默失效的地方 —— 补测的动因见
...
但日常回归只吃已入库的 fixtures，所以在无 DuckDB 依赖的环境下照样跑通。`test/sample-source.test.ts` 是这个约束下唯一新增的用例 ——
它只用合成行与一个写死的、不存在的路径，不碰数据文件，故合规。

⚠️ **分片缺失算警告不算失败**：`verify-source.ts` 遇到缺失的分片只累加「未检查分片」
并在 summary 里提示，**退出码仍为 0**。所以 **退出 0 不等于语料完整** ——
要看 summary 里的「未检查分片」是否为 0。这是刻意设计：`--year 1900` 这类
无数据的过滤必须退出 0，否则它会被误报成失败。
...
只在**需要重建基准**时手动跑（数据集缺失时会打印指引并退出；但**日常测试不需要它** ——
fixtures 已入库，跑 `npm test` 用不着数据集）。
...
└── tools/
    ├── build-fixtures.ts     从 db/ 数据集抽样重建 fixtures（手动执行）
