...
// 它们自己不写 SQL、不碰连接。
//
// ⚠️ 本模块**不进 `npm test` 的断言** —— `npm test` 必须继续在「无 db/ 数据集、
 ...
 * SQL 字符串字面量。
 *
 * ⚠️ 必须是**字面量**而非占位符：`read_parquet()` 的路径参数在绑定阶段就要求常量，
 *    传 `?` 会被 DuckDB 拒绝。所以路径只能自己拼进 SQL，引号也必须自己转义。
 */
...
export async function openSource(dbDir: string = DB_DIR): Promise<DuckDBConnection> {
	const cached = connections.get(dbDir);
	...
	const pending = (async (): Promise<DuckDBConnection> => {
		...
		// 顺带消掉了旧形态下最烦人的一类故障 —— 文件锁。内存库无锁可争，parquet 只读
		// 也不上锁，编辑器里的数据库扩展再占也影响不到这里。
	...
	pending.catch(() => connections.delete(dbDir));
	connections.set(dbDir, pending);
	return pending;
}
