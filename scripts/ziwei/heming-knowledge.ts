 ...
 * 格局命中由 `patterns.ts` 负责，四化只看**生年干**。文件内出现的「飞化互参」「大限
 * 四化」等表述属于方法论层面的描述，**不要据此去调 `sihua.ts` 的飞星派工具**
 * （见 `.claude/CLAUDE.md` 的体系硬约束）。
 *
...
/**
 * 十四主星坐夫妻宫的断语：星名 → 五段式断语（+ 可选倪师原话）。
 *
 * 键是**主星名**（十四主星全收）。空宫借对宫主星论时也走本表 ——
 * `cli/commands.ts` 传入的是「本宫主星，为空则取 `borrowedStars`」的并集，
 * 查不到时输出「（无收录断语）」而非报错。
 *
 * `ni_quote` 为可选：只有部分主星有倪师原话，缺省表示该星没有可引的原文，
 * **不是漏填**。
 */
export const STAR_IN_FUQI_GU: Record<
	string,
	{
		/** 一句话核心断语（合盘输出里的主行） */
		summary: string;
		/** 吉象的条件与表现（三方吉曜、化禄、庙旺等前提） */
		good: string;
		/** 凶象与注意事项（化忌、落陷、会煞、特定宫位组合等） */
		bad: string;
		/** 配偶的外形与性格描述 */
		spouse_traits: string;
		/** 婚期建议（早晚婚、年龄差等） */
		timing: string;
		/** 倪海夏原话（可选，缺省表示该星无可引原文） */
		ni_quote?: string;
	}
> = {
	紫微: {
	...
	廉贞: {
		...
		ni_quote:
			"廉贞贪狼/廉贞破军在夫妻宫，不管男命女命，非生离即死别；廉贞七杀——半路埋尸；廉贞破军——水中做冢；廉贞贪狼——横夭（夭折）",
	},
	天府: {
	...
	贪狼: {
		...
		ni_quote:
			"贪狼除了指桃花星，也指酒色财气赌，统统在贪狼里面；贪狼在午宫是武官星，不要随便批桃花",
	},
	巨门: {
	...
	七杀: {
		...
		ni_quote:
			"七杀居夫妇而鸳衾半冷（古赋）；如果你娶个太太是七杀入命，那你就差不多毁了一半了，很累啊，草木皆兵",
	},
	破军: {
...
/**
 * 四化落夫妻宫的断语：`化禄` / `化权` / `化科` / `化忌` → 一段断语。
 *
 * 键固定为四个，且**带「化」字前缀**（不是 `SiHua` 的裸值「禄」「权」「科」「忌」）——
 * `cli/commands.ts` 用模板串拼键（`SIHUA_IN_FUQI_GU[\`化${x.hua}\`]`），
 * 改键名会让查表静默落空。
 *
 * 覆盖的是**生年四化**落夫妻宫的情形（本项目只认生年干四化）。
 */
...
/**
 * 合盘分析核心方法论全文（Markdown 文本，十个章节）。
 *
 * 内容：双宫联参原则（夫妻宫 + 福德宫）、天作之合的判断标准、完整合盘五步法、
 ...
 * 不影响任何判定分支。
 *
 * ⚠️ 文中「四化飞化互参」等表述是方法论层面的技法描述；本项目已下线飞星派的
 * 宫干自化与大限四化取宫干，**不要**把它当成去调 `sihua.ts` 飞星工具的依据
 * （见 `.claude/CLAUDE.md` 的体系硬约束）。
 */
...
- 巨门在兄弟/交友宫：倪师：”巨门在朋友宫，代表跟朋友合伙会朋友变仇人”
 ...
 * 婚姻相关杂曜的一句话释义：星名 → 释义。
 *
 * 收 9 颗桃花/孤克类星（红鸾、天喜、天姚、咸池、孤辰、寡宿、天哭、天虚、天巫）。
 * `cli/commands.ts` 在合盘里遍历双方**夫妻宫的全部星曜**（含杂曜，不只主星），
 * 命中本表才输出一行，一条都没命中时输出「双方夫妻宫无收录的桃花/孤克星」
 * —— 故本表是**稀疏收录**：没收的星就是不提示，不是漏填。
 *
 * ⚠️ 只是文案，不参与格局判定。
 */
